...
Once the `conda-forge/label/llvm_rc` channel has been enabled, `libmlir, libmlir18, mlir` can be installed with `conda`:

```
conda install libmlir libmlir18 mlir
```

or with `mamba`:

```
mamba install libmlir libmlir18 mlir
```
